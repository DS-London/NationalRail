...
class Request
{
	...
	abstract XmlNode data();

	abstract bool processResponse(XmlNode nodeResponse);

	bool _loadToken()
	{
		if( _token.length == 0 )
		{
			auto f = File("token.txt", "r");
			_token = f.readln();
		}
...
class DepBoardWithDetails : Request
{
	...
	{		
		...
		node.addChild("soap:Header").addChild("typ:AccessToken").addChild("typ:TokenValue",_token);
	...
	override bool processResponse(XmlNode nodeResponse)
	{
